...
import time
import micropython
from machine import Pin,PWM,SPI,Signal

# 8桁 7セグメントLEDの制御コントローラ用モジュール
# max7219_8digit : https://github.com/pdwerryhouse/max7219_8digit
# ここからmax7219_8digit.pyをPi Picoのmain.pyと同じフォルダに置く。
# 開発元のスクリプトから顔部分のｆれえどおおお
...
_BUTTON_TYPE=('A','D','R') #ボタンタイプ。それぞれ「次へ進む」「一つ戻る」「初期値にリセット」
...
def RelayOnAndDispChange(pin,onoff:bool , wired:bool=False):
    ...
    if onoff:
        ...
        g_MAX7219.write_to_buffer(g_MAX7219_DispData)
        g_MAX7219.display()
    else:
        print("OFF==",pin,tmp_g_Button_idx,onoff)
        #押されたボタンの情報部のみ空白、現在の顔番号、顔情報を表示
        g_MAX7219_DispData=" "*2+("{:>2}".format(g_FACE_NUMBER))+_FACE_LIST[g_FACE_NUMBER]
        g_MAX7219.write_to_buffer(g_MAX7219_DispData)
        g_MAX7219.display()
...
def multipurposeDispChange(msg:str):
    tmp_msg=msg[0:8]
    g_MAX7219.write_to_buffer(tmp_msg)
    g_MAX7219.display()
...
def init_():
    ...
    global g_LEDPWM
    global g_LEDHB
    #MAX7219 SPI設定。7セグの制御用。設定後対象のグローバル変数に保存。
    ...
    tmp=max7219_8digit.Display(spi=spi,ss=ss,intensity=1)
    g_MAX7219=tmp
    g_MAX7219.write_to_buffer("INITIAL-")
    g_MAX7219.display()
